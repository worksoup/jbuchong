    ...
    R: GetInstanceTrait,
{
    ...
    where
        F: Fn(T) -> R + 'static,
    {
        let call_from_java: Box<dyn Fn(InstanceWrapper) -> Result<Instance, J4RsError>> =
            Box::new(
        ...
        F: Fn(T) -> R + 'static,
    {
        ...
        let instance = raw_pointer_to_instance::<"io.github.worksoup.function.LumiaFunction">(internal_closure_raw);
        Function {
            instance,
            internal_closure_raw,
            _t: PhantomData,
